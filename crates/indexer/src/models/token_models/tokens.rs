...
// PK of current_token_listing, i.e. token_data_id_hash + property_version + lister_address, used to dedupe
pub type CurrentTokenListingPK = (TokenDataIdHash, BigDecimal, Address);
// PK of current_token_listing, i.e. token_data_id_hash + property_version + owner_address, used to dedupe
pub type CurrentTokenEscrowPK = (TokenDataIdHash, BigDecimal, Address);
// PK of current_token_listing, i.e. token_data_id_hash + property_version + to/from_address, used to dedupe
...
impl Token {
    ...
    pub fn from_transaction(
        ...
        HashMap<CurrentTokenListingPK, CurrentTokenListing>,
        ...
        if let APITransaction::UserTransaction(user_txn) = transaction {
            ...
            let mut current_collection_datas: HashMap<TokenDataIdHash, CurrentCollectionData> =
                HashMap::new();
            let mut current_token_listings: HashMap<CurrentTokenListingPK, CurrentTokenListing> =
                HashMap::new();
            ...
            for wsc in &user_txn.info.changes {
                ...
                let (maybe_current_token_claim, maybe_current_token_listing, maybe_token_escrow) =
                    match wsc {
                        APIWriteSetChange::WriteTableItem(write_table_item) => (
                            ...
                            .unwrap(),
                            CurrentTokenListing::from_write_table_item(
                                write_table_item,
                                txn_version,
                                &table_handle_to_owner,
                            )
                            .unwrap(),
                            TokenEscrow::from_write_table_item(
                                write_table_item,
                                txn_version,
                                &table_handle_to_owner,
                            )
                        ...
                        APIWriteSetChange::DeleteTableItem(delete_table_item) => (
                            ...
                            .unwrap(),
                            CurrentTokenListing::from_delete_table_item(
                                delete_table_item,
                                txn_version,
                                &table_handle_to_owner,
                            )
                ...
                if let Some(listing) = maybe_current_token_listing {
                    current_token_listings.insert(
                        (
                            listing.token_data_id_hash.clone(),
                            listing.property_version.clone(),
                            listing.lister_address.clone(),
                        ),
                        listing,
                    );
                }
                if let Some((escrow, maybe_current_escrow)) = maybe_token_escrow {
                    token_escrows.push(escrow);
                    if let Some(current_escrow) = maybe_current_escrow {
                        current_token_escrows.insert(
                            (
                                current_escrow.token_data_id_hash.clone(),
                                current_escrow.property_version.clone(),
                                current_escrow.owner_address.clone(),
                            ),
                            current_escrow,
                        );
                    }
    ...
    pub fn from_write_table_item(
        ...
        let maybe_token = match TokenWriteSet::from_table_item_type(
            ...
            Some(TokenWriteSet::TokenEscrow(inner)) => Some(inner.token),
            _ => None,
        };
...
impl TableMetadataForToken {
    ...
    fn get_table_handle_to_owner(
        ...
        let table_handle: TableHandle = match TokenResource::from_resource(
            ...
            TokenResource::TokenListingsResource(inner) => inner.listings.handle,
